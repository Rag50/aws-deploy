...
app.post('/api/change-style', upload.single('video'), async (req, res) => {
    try {
        ...
        const videoDuration = await getVideoDuration(videoPath); // Moved video duration check earlier

        // Handle watermark based on user type and video duration
        let applyWatermark = userdata.usertype === 'free' && videoDuration > 600;
        if (applyWatermark) {
        ...
        // Determine watermark stream index if applicable
        const watermarkStreamIndex = applyWatermark ? 1 : -1;

        await new Promise((resolve, reject) => {
            let filterComplex = '';

            // Sound effects and audio mixing filters
            if (soundEffects.length > 0) {
                filterComplex += soundEffects.map((effect, index) =>
                    `[${inputs.length + index}:a]adelay=${effect.timestamp}|${effect.timestamp}[sfx${index}]`
                ).join('; ') + '; ';
                filterComplex += `[0:a]${soundEffects.map((_, index) => `[sfx${index}]`).join('')}amix=inputs=${soundEffects.length + 1}:duration=first[audioMix]`;
            } else {
                filterComplex += `[0:a][0:a]amix=inputs=2[audioMix]`;
            }

            filterComplex += '; ';

            // Video processing based on resolution
            if (videoResolution === '16:9' || videoResolution === '1:1') {
                filterComplex += `[0:v]scale=${resWidth}:${resheight}:force_original_aspect_ratio=decrease,pad=${resWidth}:${resheight}:(ow-iw)/2:(oh-ih)/2,setdar=${videoResolution === '16:9' ? '16/9' : '1/1'}[scaled]; `;
                filterComplex += `[scaled]subtitles="${tempassFile}":force_style='Alignment=2'[subtitled]`;
            } else {
                filterComplex += `[0:v]subtitles="${tempassFile}":force_style='Alignment=2'[subtitled]`;
            }

            // Apply watermark overlay for free users if duration > 10 mins
            if (applyWatermark) {
                filterComplex += `; [subtitled][${watermarkStreamIndex}:v]overlay=enable='gte(t,600)'[outv]`;
            } else {
                filterComplex += `; [subtitled]null[outv]`; // No watermark
            }

            // Build the FFmpeg command
            ffmpegCommand = `ffmpeg ${inputs.map(input => `-i "${input}"`).join(' ')} ${soundEffects.map(effect => `-i "${effect.file}"`).join(' ')} ` +
        ...
        // Existing code for uploading to storage, updating user data, and cleanup...
